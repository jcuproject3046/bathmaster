					...
					,items:[
						{
							...
							,columns:[
								{id:'rooms',header:'淋位',width:50,sortable:true,dataIndex:'rmId'}
		...
		this.roomDetailTemplate=new Ext.XTemplate(
			 '<div class="ux-room-details">'
			 	,'<tpl for=".">'
			 		...
			 		,'<p><b>淋位号:</b>{rmId}</p></div>'
			 		,'<div class="ux-room-details-type">'
			 		,'<p><b>淋位类型:</b>{rmCatalog}</p></div>'
			 	,'</tpl>'
			 ,'</div>'
		);
	...
	,addRoomInfo:function(selRoomRecord){	//增加房间到Grid
		...
		if(this.roomsGrid.store.indexOf(selRoomRecord)<0 && this.infoState=='init'){
			...
			switch(this.infoState){
				case 'checkout':
					Ext.ux.Toast.msg('提示','该登记信息正在进行结账，不能添加新淋位信息');
					break;
				case 'afterCheckout':
					Ext.ux.Toast.msg('提示','该登记信息已结账，不能添加新淋位信息');
					break;
				case 'afterCheckin':
					Ext.ux.Toast.msg('提示','该登记信息已保存，请重置后重新操作');
					break;
				case 'showCheckinInfo':
					Ext.ux.Toast.msg('提示','查看登记信息时不能添加新淋位');
					break;
				case 'init':
					Ext.ux.Toast.msg('提示','该淋位已在登记中');
					break;
			}
